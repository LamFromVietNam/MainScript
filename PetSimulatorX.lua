...
require(game:GetService("ReplicatedStorage").Framework.Modules.Client["5 | Message"]).New("Cam on ban vi da su dung script cua LamSol^^!")
...
-- ALL TABS --
   local Win = VLib:Window("LamSol", "Pet Simulator X")
...
workspace.__THINGS.__REMOTES:FindFirstChild("equip best pets"):InvokeServer(unpack(args))
   ...
   game.Players.LocalPlayer.Character.Humanoid:ChangeState(11)
       ...
       Misc:Button("Unlock Teleport", function()
   ...
   u:Button("LamSol#2814", function()
   
       setclipboard("https://discord.gg/dNt3eKfZ")
...
function GetMyPets()
    ...
    return returntable
       ...
       local farmspeed = {"Fast", "Slow"}
   
Autofarm:Dropdown("Select The Speed Farm for Nearest", farmspeed, function(speedF)
_G.SpeedFarm = speedF
end)

   Autofarm:Toggle("Start Farm", false, function(Farmi)
if Farmi == true then
_G.Farm = true
elseif Farmi == false then
    _G.Farm = false
end
...
if _G.SpeedFarm == 'Fast' then
    function FarmCoin(Coin, Pet)
        game.workspace['__THINGS']['__REMOTES']["join coin"]:InvokeServer({[1] = Coin, [2] = {[1] = Pet}})
        game.workspace['__THINGS']['__REMOTES']["farm coin"]:FireServer({[1] = Coin, [2] = Pet})
     end
    ...
    local closest = cointable[1]
    if closest ~= nil then
for j,k in pairs(getpet) do
coroutine.wrap(function()
    if closest ~= nil then
FarmCoin(closest.Name, k)
    end
end) ()
end
repeat task.wait(.3) until _G.Farm
else
    task.wait()
end
elseif _G.SpeedFarm == 'Slow' then
    function FarmCoin(Coin, Pet)
        game.workspace['__THINGS']['__REMOTES']["join coin"]:InvokeServer({[1] = Coin, [2] = {[1] = Pet}})
        game.workspace['__THINGS']['__REMOTES']["farm coin"]:FireServer({[1] = Coin, [2] = Pet})
     end
...
repeat task.wait(.6) until _G.Farm
else
    task.wait()
end
     end
